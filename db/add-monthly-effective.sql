-- ============================================================================
-- add-monthly-effective.sql — מוסיף "אפקטיבית מכלל העבודה" גם לרמה החודשית
-- ב-monthly_trend (עד כה monthly_trend כלל רק hours_saved ו-efficiency_pct,
-- שזו התייעלות *בתוך* משימות AI בלבד — לא לוקחת בחשבון שעבודת AI היא רק
-- חלק מכלל העבודה, בדיוק כמו שכבר טופל ברמה הארגונית וברמת הצוות).
    ...
    'monthly_trend', coalesce((
      ...
      from (
        select
          ...
          -- אפקטיבית לחודש: אותו שקלול כמו ברמה הארגונית, רק מוגבל לחודש הזה
          -- ורק לצוותים שהוגדר להם נתח AI (אחרת לא ממציאים מספר)
