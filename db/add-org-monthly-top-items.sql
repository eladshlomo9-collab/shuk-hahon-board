-- ============================================================================
-- add-org-monthly-top-items.sql — מוסיף top_items גם ל-monthly_trend הארגוני
-- הכללי (לא רק ל-by_board כמו ב-add-board-monthly-top-items.sql). הוחלט
-- שזה בטוח כי בארגון הזה כמה "teams" פנימיים שייכים לאותו לקוח/מנהל
-- ואמורים להיראות ביחד — אין כאן ערבוב בין ארגונים שונים, רק בין תת-צוותים
-- של אותו ארגון (וזה מכוון, לא דליפת מידע).
-- תוסף בטוח להרצה חוזרת (idempotent, create-or-replace בלבד). בטוח להריץ.
-- ============================================================================
...
begin
  ...
  item_rows as (
    ...
    group by i.id, i.name, ab.board_id, ab.board_name, ab.team_id, ab.team_name, ab.ai_work_share, i.created_at
  ),
  team_agg as (
  ...
  month_board_item_rank as (
    select
      to_char(created_at, 'YYYY-MM') as month,
      board_id,
      item_name,
      greatest(manual_minutes - ai_minutes, 0) as saved_min,
      row_number() over (
        partition by board_id, to_char(created_at, 'YYYY-MM')
        order by greatest(manual_minutes - ai_minutes, 0) desc
      ) as rnk
    from item_rows
    where ai_minutes is not null and manual_minutes is not null
  ),
  -- דירוג משימות ברמת הארגון כולו לכל חודש (לא מוגבל לבורד בודד) — משמש
  -- את monthly_trend הראשי, כדי להסביר קפיצות/ירידות בהתייעלות הכללית.
  month_item_rank as (
    select
      to_char(created_at, 'YYYY-MM') as month,
      item_name,
      greatest(manual_minutes - ai_minutes, 0) as saved_min,
      row_number() over (
        partition by to_char(created_at, 'YYYY-MM')
        order by greatest(manual_minutes - ai_minutes, 0) desc
      ) as rnk
    from item_rows
    where ai_minutes is not null and manual_minutes is not null
  ...
  select jsonb_build_object(
    ...
    'monthly_trend', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'top_items', coalesce((
          select jsonb_agg(jsonb_build_object(
            'item_name', mir.item_name,
            'hours_saved', round(mir.saved_min / 60.0, 1)
          ) order by mir.saved_min desc)
          from month_item_rank mir
          where mir.month = t.month and mir.rnk <= 3
        ), '[]'::jsonb)
      ) order by month)
      from (
        select
      ...
      select jsonb_agg(jsonb_build_object(
        ...
        'monthly_trend', coalesce((
          select jsonb_agg(jsonb_build_object(
            ...
            'top_items', coalesce((
              select jsonb_agg(jsonb_build_object(
                'item_name', mbir.item_name,
                'hours_saved', round(mbir.saved_min / 60.0, 1)
              ) order by mbir.saved_min desc)
              from month_board_item_rank mbir
              where mbir.board_id = mba.board_id and mbir.month = mba.month and mbir.rnk <= 3
            ), '[]'::jsonb)
          ) order by mba.month)
